...
import {
  ...
  ArrowLeft, ShieldAlert, KeyRound, Sparkles,
} from "lucide-react";
...
export default function SuccessPage({
  ...
  const cred = tool ? generateFakeCredentials(tool.id)[0] : null;

  const [showPassword, setShowPassword] = useState(true);
  const [accountUsed, setAccountUsed] = useState(false);

  // Simulate "account already in use" after 4 seconds
  useEffect(() => {
    const t = setTimeout(() => setAccountUsed(true), 4000);
      ...
      <div className="w-full max-w-md flex flex-col gap-5">
        ...
        >
          ...
          {cred ? (
            ...
            <p className="px-5 py-4 text-sm text-zinc-500">No credentials found.</p>
          )}
        </motion.div>

        {/* ── Account In Use Warning (auto-appears after 4s) ───────── */}
        {accountUsed && (
          <motion.div
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-amber-400/70 leading-relaxed">
                  ...
                  hit <span className="text-amber-300 font-medium">"Try Another Account"</span> below
                  to get a fresh set of credentials and try again.
                </p>
              </div>
            </div>
          </motion.div>
